"""
SSE models shim for funlab-flaskr.

When funlab-sse plugin is installed, import canonical models from there
to avoid duplicate entity registrations. Otherwise, use builtin definitions.
"""

# Try to import from funlab-sse plugin (canonical source)
try:
    from funlab.sse.model import (
        PayloadBase,
        EventPriority,
        EventBase,
        EventEntity,
    ...
    print("[SSE Models] Using canonical models from funlab-sse plugin")
except ImportError:
    # Fallback: builtin definitions when funlab-sse is not installed
    print("[SSE Models] Using builtin SSE models (funlab-sse not installed)")
    import ast
    ...
    class EventBase(_Readable):
        ...
        def from_entity(cls, entity: 'EventEntity'):
            ...
            event = cls(
                ...
                priority=EventPriority[entity.priority] if isinstance(entity.priority, str) else entity.priority,
    ...
    class EventEntity(EventBase):
        ...
        is_read: bool = field(init=False, default=False, metadata={'sa': Column(Boolean, nullable=False)})
